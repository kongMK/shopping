  ...
  <div class="pay">
    ...
    <div class="info" v-if="selectedAddress.address_id">
      <div class="info-content">
        ...
        <span class="moble">{{ selectedAddress.phone }}</span>
      </div>
      <div class="info-address">
        {{ longAddress }}
      </div>
    </div>
    <div class="info" v-else @click="addAddress">
      请选择配送地址
    </div>
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <!-- 买家留言 -->
      <div class="buytips">
        <textarea v-model="remark" placeholder="选填：买家留言（50字内）" cols="30" rows="10"></textarea>
      </div>
    </div>
...
<script>
import { addAddress, getAddressList } from '@/api/address'
...
export default {
  ...
  computed: {
    ...
    longAddress () {
      ...
      return Object.values(region).join(' ')
    },
    // 路由获取的参数
    mode () {
  ...
  methods: {
    ...
    // 添加地址
    async addAddress () {
      const { message } = await addAddress().then(() => {
        this.$toast(message)
      })
    },
    // 提交订单
